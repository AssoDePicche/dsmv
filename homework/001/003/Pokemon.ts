...
export async function fetchPokemon(name: string): Promise<Either<String, Pokemon>> {
  ...
  try {
    ...
    if (!response.ok) {
      return left("❌ Pokémon não encontrado!");
    }
    ...
    return left("⚠️ Erro de rede. Tente novamente.");
  }
}
